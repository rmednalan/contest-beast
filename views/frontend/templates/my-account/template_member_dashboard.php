...
<div class="labdevs_small_success" style="position: fixed; top: 70px; left: 0px; z-index: 100000; width: 100%; padding: 10px; text-align: center; height: auto; display: none;">
	<span style="color: white;background-color: red;padding: 10px 30px;border-radius: 9px;font-family: monospace;line-height: 1em;">Copied To Clipboard!</span>
</div>
<script>
jQuery(document).ready(function($) {
...
.contestbuzz-table-responsive{
box-shadow: 0 0 3px #000 !important;
}
.contestbuzz-table-responsive td a{
color: #673AB7;
}

.contestbuzz-table-responsive tr:nth-child(2n+1){
background: #ddd
}
.contestbuzz-table-responsive tr:nth-child(2n){
background: rgba(189, 189, 189, 0.66)
}
	.contestbuzz-table-responsive thead,.contestbuzz-table-responsive thead tr:first-child {
    background: #673AB7;

}
.contestbuzz-table-responsive thead th {
font-size: 11px;
color: white;
...
background: linear-gradient(to left, #42275a , #734b6d); /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */
	...
	<div class="well">
		<a href="<?php echo get_page_link(1422); ?>" class="btn btn-danger btn-lg"><span  class="glyphicon glyphicon-plus"></span>  Add contest</a>
		<button type="button" class="btn btn-default pull-right" >Stats Overview <span class="badge"><?php echo labdevs_get_current_author_number_of_custom_posts( 'contest' ); ?></span> </button>
	</div>
	
	
	<h1 class="page-header"><span class="glyphicon glyphicon-stats" style="color:#673AB7;"></span> Your Contests</h1>
	<div id="updating_table" class="alert alert-info" style="display: none;padding: 10px;">
		<strong>Updating Table!</strong> Please wait.
			...
			<table class="table  dataTable no-footer contestbuzz-table-responsive" style="text-align:center;">
				<thead>
				<tr>
					<th style=" vertical-align:middle;">Title</th>
					<th style="text-align:center; vertical-align:middle;">PUBLISHED DATE </th>
				 ...
				 */
				 ...
				 ?>
					 <tr>
						<td style="width:130px; vertical-align:middle; text-align:left; text-transform:capitalize"><?php echo substr(get_the_title(),0,30); ?></td>
					...
					printf('<br /><a href="%s">%s</a>', $nonce_url, __('Export Entrants')); ?></td>
						...
						<button class="the_clipboard btn btn-default btn-sm" style="background-color:#607d8b;color:white" ><span  class="glyphicon glyphicon-paperclip"></span></button>
						 <a href="<?php the_permalink(); ?>" role="button" class="btn btn-default btn-sm" style="background-color:#00bcd4;color:white"><span class="glyphicon glyphicon-eye-open"></span></a>
						 ...
						 <button 
							...
							data-contest_video="<?php echo esc_attr($items["contest_video"]);  ?>"
							...
							data-youtube_video="<?php echo esc_attr($items["youtube_video"]);  ?>"
							data-user_scripts="<?php echo esc_attr($items["user_scripts"]);  ?>"
							...
							class="update_spy_link_button btn btn-default btn-sm" style="background-color:#4caf50;color:white">
								<span class="glyphicon glyphicon-edit"></span>
			 					</button>
							<button
								data-id="<?php the_ID(); ?>"
								class="delete_spy_link_button btn btn-default btn-sm" style="background-color:red;color:white">
		...
		jQuery(document).ready(function($){
			...
			$.ajax({
					...
					url: "<?php echo admin_url('admin-ajax.php'); ?>",
					data: {
						action: 'get_post_info',postid:id,
					},
					success: function(data) {
						//console.log("test submition");
				...
				var contest_video = $(this).attr('data-contest_video');
				...
				var user_scripts = $(this).attr('data-user_scripts');
				...
				var user_scripts, user_scripts_inputid = 'user_scripts';
				var user_scripts_editor = tinyMCE.get(user_scripts_inputid);
				if (editor) {
					user_scripts_editor.setContent(user_scripts);
				} else {
					$('#'+user_scripts_inputid).val(user_scripts);
				}
				...
				$('#contest_video').val( contest_video );
				$('#youtube_video').val( youtube_video );
				$('#user_scripts').val( user_scripts );
			...
			$(document).on('click','#update_spy_link',function( e ){
				...
				var user_scripts, user_scripts_inputid = 'user_scripts';
				var user_scripts_editor = tinyMCE.get(user_scripts_inputid);
				if (user_scripts_editor) {
					user_scripts = user_scripts_editor.getContent();
				} else {
					user_scripts = $('#'+user_scripts_inputid).val();
				}
	...
	<div class="modal-content">
		...
		<div class="row"><div class="col-md-8 col-md-offset-2">
			<fieldset>
				...
				<!-- Start Text input for Custom Rules-->
				<div class="form-group">
				...
				<!-- End Text input for Custom Rules-->
				...
				<!-- Start Text input for Custom Tracking Scripts-->
				<div class="form-group">
					...
					<small style="">This contest has custom tracking scripts (optional)</small>
					</div>
				</div>
				<div class="form-group open_scripts" style="display:none;" data-dependency="contest-beast[tracking-scripts-custom]" data-dependency-value="yes">
					<div class="">
					 <textarea id="scripts" name="textarea" rows="7" placeholder="Enter your scripts"  class="form-control"></textarea>
					</div>
				</div>
				<!-- End Text input for Custom Tracking Scripts-->
				
				<!--div class="form-group">
					<label class="control-label" for="textinput">Video Advertisement</label>
					<div class="">
					<input type="text" value="" name="contest-beast[contest-video]" id="contest_video" class="form-control input-md">
					<p> Copy the Video ID of your desired video post (example: https://www.youtube.com/watch?v=l5jNyT_dsoc) this is the Video id v=l5jNyT_dsoc</p>
					</div>
				...
				<!-- Giveaway Item Picture and Description -->
				<div class="form-group">
					<div class="row">
						<div class="col-md-6">
								<label class="control-label" for="">Giveaway Item Picture</label>
								</br>
								Giveaway Item Picture
								<!--<form enctype="multipart/form-data">
						...
						<div class="col-md-6">
							...
							<br/>
							Giveaway Item Picture Description
						</div>
					</div>
				</div>
