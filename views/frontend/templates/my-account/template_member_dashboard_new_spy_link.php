...
<div class="col-md-8 col-md-offset-2 ">
	<div class="well">
		<a href="<?php echo get_page_link(9); ?>" class="btn btn-danger btn-lg"><span  class="glyphicon glyphicon-stats"></span>  Your contests</a>
	</div>
	<h1 class="page-header"><span class="glyphicon glyphicon-target" ></span><i class="fa fa-edit" style="color:#673AB7"></i>  New Contest</h1>
...
<div class="required_notification" >This field is required!</div>
		...
		<!--video url-->
		<!--div class="form-group">
		  <label for="youtube_video">Add Youtube video <code>Copy embed code from youtube and paste it here</code></label>
		...
		<!--<div class="form-group">
			<div class="row">
				<div class="col-md-12 col-sm-12">
					<label class="control-label" for="">Custom CSS Background</label>
						<form enctype="multipart/form-data">
								<?php //add_thickbox(); ?>
									<div id="thickbox_modal" style="display:none; padding: 0 0 0 10px;">
									<label style="font-size: 20px; padding:5px 0 5px 0;" for="">Custom CSS Background</label>
										<br/>
										<label class="control-label" for="">Class: &nbsp&nbsp </label>
										<input list="contest_custom_css_list" id="contest_custom_css_list_val" required autocomplete="on" >
										<datalist id="contest_custom_css_list">
										  <option value="Header">
										  <option value="Body">
										  <option value="Footer">
										</datalist><input type="button" name="testbutton" id="testbutton" value="Test">
										<script type="text/javascript">
											(function ($) {
												$(document).ready(function () {
													$( "#testbutton" ).click(function(event) {
														var a = $('#header').val();
														$('#contest_custom_css_field').data("sample", a);
														alert($('#contest_custom_css_field').data("sample"));
														//alert(a);
														//$('#custom_css_field_desc').data(
														//$('#custom_css_field_desc').val($('#contest_custom_css_field').val());
													});
												});	
											})(jQuery);
										</script>
										<input type="hidden" name="header" id="header" value="text" >
										<input type="hidden" name="body" id="body">
										<input type="hidden" name="footer" id="footer">
										<div class="contest_custom_css_list_error1" style="display:none; color: red;" hidden="hidden">
											*This Field is Required
										</div>
										<br/>
										<textarea id="contest_custom_css_field" name="textarea" rows="5" placeholder="Enter your attributes and value"  class="form-control" required></textarea>
										<div class="contest_custom_css_list_error2" style="display:none; color: red;" hidden="hidden">
											*This Field is Required
										</div>
										<label style="font-size: 12px; padding:5px 0 5px 0; color: red;" for="">NOTE: &nbsp&nbsp If you dont have any idea in CSS, Please don't use this function or it will be have errors in the web page.</label>
										<div class="modal-footer">
										<br/>
										<button type="button" class="btn btn-default" data-dismiss="modal" name="contest_custom_css_close" id="contest_custom_css_close">Close</button>
										<button type="button" class="btn btn-primary" name="contest_custom_css_save" id="contest_custom_css_save" data-dismiss="modal" aria-hidden="true">Save changes</button>
										</div>
									</div>
								<a href="#TB_inline?width=700&height=400&inlineId=thickbox_modal" class="thickbox" style="color: black;"><button>Add Custom CSS</button></a>
								<br/>
								<br/>
								<div id="custon_css_field" style="display:none; padding: 0 0 0 10px;">
									<label style="font-size: 15px; padding:5px 0 5px 0;" for="">Custom CSS List</label>
									<div class="custom_css_field_value" id="custom_css_field_value">
									<table class="custom_css_field_value_list" id="custom_css_field_value_list">
										<thead>
											<th>Class</th>
										...
										(function ($) {
											$(document).ready(function () {
												$("#contest_custom_css_close").click(function(event){
														$(".contest_custom_css_list_error1").hide();
														$(".contest_custom_css_list_error2").hide();
														$('#TB_window').fadeOut();
														tb_remove();
												});
											});	
										...
										(function ($) {
											$(document).ready(function () {
												$("#contest_custom_css_save").click(function(event){
													if($("#contest_custom_css_list_val").val() == ''){
														$(".contest_custom_css_list_error1").show();
													}
													else if($("#contest_custom_css_field").val() == ''){
														$(".contest_custom_css_list_error1").hide();
														$(".contest_custom_css_list_error2").show();
													}else{
														$(".contest_custom_css_list_error2").hide();
														//alert($("#contest_custom_css_list_val").val()+" "+$("#contest_custom_css_field").val());
														$('#TB_window').fadeOut();
														tb_remove();
														$('#custon_css_field').show();
														var custom_val1 = $('#contest_custom_css_list_val').val();
														var custom_val2 = $('#contest_custom_css_field').val();
														var records = [
															{key: custom_val1, key2: custom_val2},
														];
														for(var i = 0; i < records.length; i++) {
															var row = $('<tr></tr>');
															row.append($('<td></td>').text(records[i].key));
															row.append($('<td></td>').text(records[i].key2));
															$('#custom_css_field_value_list').append(row);
															$('#custom_css_field_value').append("<input type='text' value='"+records[i].key+"' id='custom_css_field_class' class='custom_css_field_class'><br/>");
															$('#custom_css_field_value').append("<input type='text' value='"+records[i].key2+"' id='custom_css_field_desc' class='custom_css_field_desc'><br/>");
														}
														var headerclass = $('#custom_css_field_classe').val();
														var headerdesc = $('#custom_css_field_desc').val();
														if(headerclass == "Header"){
															var head_val = headerdesc;
															$('#header').data("sample", a);
															var a1 = $('#header').data("sample");
															
															
															alert($('#contest_custom_css_field').data("sample"));
														}
														$('#contest_custom_css_field').val('');
														$('#contest_custom_css_list_val').val('');
													}
												});
											});
										})(jQuery);
...
background: linear-gradient(to left, #6441A5 , #2a0845); /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */
		...
		jQuery(document).ready(function ($) {
			...
			$(document).on('click', '#create_spy_link', function (e) {
				...
				var contest_video = $(this).attr('#contest_video');
				...
				var contest_video = $('#contest_video').val();
				var youtube_video = $('#youtube_video').val();
				var user_scripts = $('#user_scripts').val();
				//var custom_css_class = $('#custom_css_field_class').val(); //added 4/7/2017
				//var custom_css_desc = $('#custom_css_field_desc').val(); //added 4/7/2017
				var custom_mailing = $('#custom_mailing').val();
				var contest_featured_image = "";
				...
				if ($('#contest_featured_image_url').val() != "") {
	
					contest_featured_image = $('#contest_featured_image_url').val();
				}  
				
				//dito
				//if ($('#picture_desc').val() != "") {
	
				//	picture_description = $('#picture_desc').val();
				//}  
				...
				$.ajax({
					...
					data : {
						...
						logo : logo,
						featured_image : contest_featured_image,
						//custom_css_class : custom_css_class, //new added 4/6/17
						//custom_css_desc : custom_css_desc, //new added 4/6/17
